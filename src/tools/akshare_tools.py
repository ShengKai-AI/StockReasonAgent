import akshare as ak
import pandas as pd
from datetime import datetime
from typing import List, Dict, Optional, Union

class AkShareTool:
    def __init__(self):
        print("📡 初始化 A股数据雷达 (基于东方财富接口)...")

    def _clean_symbol(self, symbol: str) -> str:
        """
        清洗代码格式：把 'SH600000' 转化为 '600000'
        因为东方财富接口通常只需要 6 位数字代码
        """
        return symbol.replace("SH", "").replace("SZ", "").replace("bj", "")

    def _add_prefix(self, symbol: str) -> str:
        """
        为 6 位纯数字代码添加市场的 prefix (SH/SZ)
        简单规则:
        60xxxx, 68xxxx -> SH
        00xxxx, 30xxxx -> SZ
        """
        clean = self._clean_symbol(symbol)
        if clean.startswith("6"):
            return f"SH{clean}"
        elif clean.startswith("0") or clean.startswith("3"):
            return f"SZ{clean}"
        return clean

    def get_stock_concepts(self, symbol: str) -> List[str]:
        """
        功能：获取个股的热门概念关键词（取热度 Top 3）
        """
        try:
            target_code = self._add_prefix(symbol)
            df = ak.stock_hot_keyword_em(symbol=target_code)
            if df.empty:
                return []
            
            ignore_list = [
                "长江三角", "珠江三角", "融资融券", "沪股通", "上证180", 
                "行业龙头", "标准普尔", "富时罗素", "证金持股"
            ]
            
            # 按热度降序排列
            if '热度' in df.columns:
                df = df.sort_values(by='热度', ascending=False)
            
            concepts = df['概念名称'].head(3).tolist()
            concepts = [c for c in concepts if c not in ignore_list]
            return concepts
        except Exception as e:
            print(f"⚠️ 概念数据获取失败 ({symbol}): {e}")
            return []

    def get_stock_raw_data(self, symbol: str) -> Optional[Dict]:
        """
        获取纯净的原始数据 (Float 类型)，用于 Dispatcher 计算
        """
        clean_code = self._clean_symbol(symbol)
        try:
            # 1. 实时行情
            df_hist = ak.stock_zh_a_hist(symbol=clean_code, period="daily", start_date="20260101", adjust="qfq")
            if df_hist.empty:
                return None
            latest = df_hist.iloc[-1]
            
            # 2. 静态画像
            info_df = ak.stock_individual_info_em(symbol=clean_code)
            info_dict = dict(zip(info_df['item'], info_df['value']))
            
            # 3. 概念 (Top 3)
            concepts = self.get_stock_concepts(clean_code)

            return {
                "symbol": clean_code,
                "name": info_dict.get('股票简称', 'Unknown'),
                "price": float(latest['收盘']),
                "change_pct": float(latest['涨跌幅']),
                "sector": info_dict.get('行业'),
                "concepts": concepts,  # List[str]
                "date": str(latest['日期'])
            }
        except Exception as e:
            print(f"❌ 获取原始数据错误: {e}")
            return None

    def get_sector_change(self, sector_name: str) -> float:
        """获取行业板块涨跌幅"""
        try:
            df = ak.stock_board_industry_name_em()
            target = df[df['板块名称'] == sector_name]
            if not target.empty:
                return float(target.iloc[0]['涨跌幅'])
            return 0.0
        except:
            return 0.0

    def get_concept_change(self, concept_name: str) -> float:
        """获取概念板块涨跌幅"""
        try:
            df = ak.stock_board_concept_name_em()
            target = df[df['板块名称'] == concept_name]
            if not target.empty:
                return float(target.iloc[0]['涨跌幅'])
            return 0.0
        except:
            return 0.0

    # --- 兼容旧版方法 (尽量使用内部 helper) ---

    def get_stock_realtime_data(self, symbol: str) -> Union[Dict, str]:
        """
        [兼容旧版] 获取 实时行情 + 基础画像
        """
        clean_code = self._clean_symbol(symbol)
        try:
            print(f"🔍 正在扫描市场数据: {clean_code}...")
            df_hist = ak.stock_zh_a_hist(symbol=clean_code, period="daily", start_date="20260101", adjust="qfq")
            if df_hist.empty:
                return f"❌ 未找到代码 {clean_code} 的数据"
            
            latest_data = df_hist.iloc[-1]
            info_df = ak.stock_individual_info_em(symbol=clean_code)
            info_dict = dict(zip(info_df['item'], info_df['value']))

            agent_context = {
                "股票名称": info_dict.get('股票简称', 'Unknown'),
                "股票代码": symbol,
                "当前价格": latest_data['收盘'],
                "今日涨跌幅": f"{latest_data['涨跌幅']}%",
                "今日换手率": f"{latest_data['换手率']}%", 
                "成交量": f"{latest_data['成交量']} 手",
                "所属行业": info_dict.get('行业'),
                "总市值": f"{info_dict.get('总市值')} 元",
                "数据日期": str(latest_data['日期']),
                "热门概念":  ", ".join(self.get_stock_concepts(symbol))
            }
            return agent_context
        except Exception as e:
            return f"❌ 数据获取失败: {str(e)}"

    def get_market_context(self, stock_symbol: str, stock_info: Dict) -> Dict:
        """
        [兼容旧版] 获取板块数据并进行【异动判断】
        """
        industry_name = stock_info.get("所属行业")
        try:
            stock_change_str = str(stock_info.get("今日涨跌幅", "0")).replace("%", "")
            stock_change = float(stock_change_str)
        except:
            stock_change = 0.0
        
        print(f"📊 正在比对板块效应: {stock_symbol} ({industry_name}) vs 行业指数...")

        try:
            sector_change = self.get_sector_change(industry_name)
            
            divergence = stock_change - sector_change
            is_resonance = abs(divergence) < 2.5 
            
            if is_resonance:
                reasoning = "板块共振"
                search_strategy = f"重点搜索 '{industry_name}' 行业的近日政策、原材料价格或宏观消息，无需过度关注个股特定新闻。"
            else:
                reasoning = "独立行情"
                status = "强势领涨" if divergence > 0 else "弱势领跌"
                search_strategy = f"该股走势与板块偏离 {divergence:.2f}% ({status})。必须重点搜索 '{stock_info['股票名称']}' 的个股公告、研报或传闻。"

            return {
                "板块名称": industry_name,
                "板块涨跌幅": f"{sector_change}%",
                "个股涨跌幅": f"{stock_change}%",
                "相对强弱": f"{divergence:.2f}%",
                "异动类型": reasoning,
                "搜索策略建议": search_strategy
            }
        except Exception as e:
            return {"Error": f"板块分析失败: {str(e)}"}
